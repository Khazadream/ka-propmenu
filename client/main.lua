...
local function SpawnPed(coords, model, targetLabel, targetIcon, targetDistance, itemListName, shopNumber)
    RequestModel(model)
    while not HasModelLoaded(model) do
        Wait(0)
    end
...
local function initPedAndTarget()
    for k, v in pairs(Config.Shops) do
        SpawnPed(v.coords, v.model, v.targetLabel, v.targetIcon, v.targetDistance, v.shopItemName, k)
    end
end
