...
function GetSortPropCategories(itemList)
    local sortedCategories = {}
    for category, _ in pairs(itemList) do
...
function CreateShopMenu(actualShop)
    local currentShop = Config.Shops[actualShop]
    local shopItemList = Config.ShopItemList[currentShop.shopItemName]
    -- Create menu items for each prop
    for _, key in ipairs(GetSortPropCategories(shopItemList)) do
        local props = shopItemList[key]
